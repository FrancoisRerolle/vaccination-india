...
# @Description - This code modifies the.table1.internal and stats.default functions of the table1
# package to include survey weights. All other functions in the package are
# untouched but need to be loaded as well. "# FR modification" indicates were code was modified
...
stats.default <- function(x, weights, quantile.type=7, ...) {
  if (is.logical(x)) {
    x <- factor(1-x, levels=c(0, 1), labels=c("Yes", "No"))
  }
  if (is.factor(x) || is.character(x)) {
    y <- table(x, useNA="no")
    y.w <- wtd.table(x, weights, na.rm = T) # FR modification
    sum.y.w <- sum(y.w$sum.of.weights) # FR modification
    y.w2 <- y  # FR modification
    y.w2[y.w$x] <- y.w$sum.of.weights  # FR modification
    nn <- names(y)
    nn[is.na(nn)] <- "Missing"
    names(y) <- nn
    mapply(function(z, z.w) list(FREQ=z, PCT=100*z.w/sum.y.w), y, y.w2, SIMPLIFY = F) # FR modification
    ...
    list(
      ...
      MEAN=weighted.mean(x, weights, na.rm=TRUE), # FR modification
      SD=sqrt(wtd.var(x, weights, na.rm=TRUE)), # FR modification
      ...
      MEDIAN=weighted.median(x, weights, na.rm=TRUE), # FR modification
      ...
      q10=q["10%"],
      q25=q["25%"],
      q25w = weighted.quantile(x, weights, probs = 0.25, na.rm = T, type = 4), # FR modification
      q50=q["50%"],
      q75=q["75%"],
      q75w = weighted.quantile(x, weights, probs = 0.75, na.rm = T, type = 4), # FR modification
      q90=q["90%"],
      q95=q["95%"],
      q97.5=q["97.5%"],
      q99=q["99%"],
  ...
  if (transpose) {
    ...
    contents <- lapply(names(labels$variables), function(v) {
      ...
      y <- do.call(cbind, lapply(x, function(s) {
        ...
        render(x=z, weights = s$Women_Sampling_Weight, variable = v, name=v, missing=any.missing[v], ...)})) # FR modification
